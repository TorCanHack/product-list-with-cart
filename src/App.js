    ...
    switch(action.type) {
        case 'ADD':
            ...
            if (itemIndex > -1){
                ...
                newCart = [...state.cart, { name: action.payload.name, price: action.payload.price, quantity: 1, image: action.payload.image}];
            }
            return {
                ...state,
                cart: newCart,
...
const App = () => {
    ...
    return (
      <main className={`bg-rose-50 h-32 font-redHatText p-8 ${order ? 'overflow-hidden' : ''}`}>

        { order && (
          <>
            <div  className={'absolute inset-0 w-full h-full  bg-black opacity-60 mx-auto my-0 z-10'}></div>
            <article id='overlay' className='absolute inset-x-0 top-24 bg-white z-20 h-auto w-full p-4 rounded-t-2xl'>
              ...
              <ul className='bg-red-50'>
                ...
                <li key={index} className="border-b border-brown-100 mb-4 pb-4 px-4">
                  <div className='flex flex-row justify-between'>
                    ...
                    <img src={item.image.thumbnail} alt={`${item.name} thumbnail`} className="w-12 h-12"/>

                    <div className='flex flex-col'>
                      <span className="  text-brown-900 truncate overflow-hidden whitespace-nowrap ">{item.name} </span>
                      <div>
                    ...
                    <span className="relative top-4  ml-1 text-lg font-semibold">${(item.quantity * item.price)}</span> 
              ...
              <button  className='block bg-red-500 w-80 h-14 rounded-3xl mt-4 mx-auto' onClick={() => handleNewOrder()}>Start New Order</button>
            </article>
          </>
        ...
        <section className={`${order ? 'max-h-16 overflow-y-auto' : ''}`}>
        ...
        <Cart cart={state.cart} total={state.total} totalQuantity={totalQuantity} handleDecrement={handleDecrement} setOrder={setOrder}/>
